...
import { spokenNumbers, alignCaptions, scheduleNarration, placedWords, Word } from './speech';
import './editor.css';
...
const syncKey = (d: Draft) => JSON.stringify([voiceKey(d),d.plan.voiceSpeed,d.plan.captions.map(c=>[c.text,c.source]),d.videos.map(f=>f&&[f.name,f.size,f.lastModified])]);
...
async function analyze(blob:Blob):Promise<Word[]>{
...
export function VideoEditor({ visible, open }: { visible: boolean; open: () => void }) {
  ...
  const createVoice = () => run(async () => {
    ...
    if(lengths.some((_,i)=>!d.videos[i]))throw Error('Kling 영상 파일을 먼저 넣어주세요.');
    ...
    for(let i=0;i<lengths.length;i++){setMessage(`Kling 대사 시간 확인 중 · ${i+1}/${lengths.length}`);const words=await analyze(d.videos[i]);originalWords.push(...words.map(w=>({...w,start:Math.min(d.plan.duration,w.start+offset),end:Math.min(d.plan.duration,w.end+offset)})).filter(w=>w.end>w.start));offset+=lengths[i];}
    let dialogue=d.plan.captions.filter(c=>c.source==='dialogue');
    const alignedDialogue=alignCaptions(dialogue,originalWords);
    const dialogueRanges=originalWords.map(w=>({start:Math.max(0,w.start-0.04),end:Math.min(d.plan.duration,w.end+0.04)}));
    ...
    let narrationWords:Word[]=[];
    if(d.plan.narration.trim()){setMessage('해설 음성과 자막 시간을 연결하고 있습니다.');narrationWords=await analyze(working.voiceBlob);if(!narrationWords.length)throw Error('해설 음성에서 발화를 찾지 못했습니다.');}
    const segments=scheduleNarration(narrationWords,d.plan.duration,dialogueRanges,d.plan.voiceSpeed);
  ...
  const makeVideo = () => run(async () => {
    ...
    const size = d.videos.reduce((n, f) => n + (f?.size || 0), d.plan.narration.trim()?(d.voiceBlob?.size||0):0);
    ...
    <div className="ori-editor-inner">
      ...
      <fieldset disabled={busy || !ready}>
        ...
        <article><h3>2. 해설 음성과 자동 싱크</h3><p>등장인물 대사는 Kling 원본 소리를 사용합니다. 아래에는 해설만 적으세요. 해설이 없는 영상은 비워두면 됩니다.</p>
          ...
          {audioURL && d.plan.narration.trim() && <div className="ori-audio"><audio controls src={audioURL} onLoadedMetadata={e => setAudioLength(e.currentTarget.duration)} /><a href={audioURL} download="나레이션.wav">음성 다운로드</a>{!voiceCurrent && <p>대본 또는 목소리가 바뀌었습니다. 음성을 다시 만들어주세요.</p>}</div>}
        ...
        <article><h3>3. 자막과 첫 화면 문구</h3><p>음성에서 찾은 발화 시간으로 자동 정렬합니다. 확인 표시가 있는 문장만 검토하세요. 등장인물 대사 구간의 원본 소리는 유지됩니다.</p>
          {d.plan.captions.map((c, i) => <div className="ori-caption" key={i}><label>자막 종류<select value={c.source||'narration'} onChange={e=>change({captions:d.plan.captions.map((r,n)=>n===i?{...r,source:e.target.value as any}:r)})}><option value="narration">해설</option><option value="dialogue">등장인물 대사</option><option value="screen">화면 문구 · 시간 유지</option></select></label>{c.review&&<p className="ori-note">{c.review}<button onClick={()=>change({captions:d.plan.captions.map((r,n)=>n===i?{...r,review:undefined}:r)})}>이 자막 확인했음</button></p>}<label>시작(초)<input type="number" min="0" max={d.plan.duration} step="0.1" value={Number(c.start.toFixed(2))} onChange={e => change({ captions: d.plan.captions.map((r, n) => n === i ? { ...r, start: +e.target.value } : r) })} /></label><label>끝(초)<input type="number" min="0" max={d.plan.duration} step="0.1" value={Number(c.end.toFixed(2))} onChange={e => change({ captions: d.plan.captions.map((r, n) => n === i ? { ...r, end: +e.target.value } : r) })} /></label><label>자막<textarea rows={2} maxLength={160} value={c.text} onChange={e => change({ captions: d.plan.captions.map((r, n) => n === i ? { ...r, text: e.target.value } : r) })} /></label><button onClick={() => change({ captions: d.plan.captions.filter((_, n) => n !== i) })}>삭제</button></div>)}
