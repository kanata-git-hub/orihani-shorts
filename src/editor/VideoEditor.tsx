    ...
    <div className="ori-editor-inner">
      ...
      <fieldset disabled={busy || !ready}>
        ...
        <article><h3>3. 자막과 첫 화면 문구</h3><p>음성에서 찾은 발화 시간으로 자동 정렬합니다. 확인 표시가 있는 문장만 검토하세요. 등장인물 대사 구간의 원본 소리는 유지됩니다.</p>
          {d.plan.captions.map((c, i) => <div className="ori-caption" key={i}><label>자막 종류<select value={c.source||'narration'} onChange={e=>change({captions:d.plan.captions.map((r,n)=>n===i?{...r,source:e.target.value as any}:r)})}><option value="narration">해설</option><option value="dialogue">등장인물 대사</option><option value="screen">화면 문구 · 시간 유지</option></select></label>{c.review&&<p className="ori-note">{c.review}<button onClick={()=>change({captions:d.plan.captions.map((r,n)=>n===i?{...r,review:undefined}:r)})}>이 자막 확인했음</button></p>}<label>시작(초)<input type="number" min="0" max={d.plan.duration} step="0.1" value={c.start} onChange={e => change({ captions: d.plan.captions.map((r, n) => n === i ? { ...r, start: +e.target.value } : r) })} /></label><label>끝(초)<input type="number" min="0" max={d.plan.duration} step="0.1" value={c.end} onChange={e => change({ captions: d.plan.captions.map((r, n) => n === i ? { ...r, end: +e.target.value } : r) })} /></label><label>자막<textarea rows={2} maxLength={160} value={c.text} onChange={e => change({ captions: d.plan.captions.map((r, n) => n === i ? { ...r, text: e.target.value } : r) })} /></label><button onClick={() => change({ captions: d.plan.captions.filter((_, n) => n !== i) })}>삭제</button></div>)}
