...
The labelled room photograph is the authoritative environment reference for THIS scene. Preserve its warm light wood, cream walls, furniture design, room layout and lighting palette. Use the same clinic interior, adapting the camera angle and framing to the action. Keep the pantry's yakgwa snacks when visible. Draw only the characters requested in the scene, using their original character sheets. The room image is NOT a character sheet. It overrides conflicting room descriptions and any previous scene; preserve character identity from the ORIGINAL character sheets. Do not invent a different room or extra machinery. This selects the setting only; follow the current scene's action and props.`;
}
